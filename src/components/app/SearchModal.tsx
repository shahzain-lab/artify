...
import { Backlog, Comment, Cross, Document, GetStarted, Image, Project, Resource, UserPlus } from '@/utils/icons';
...
const SearchModal: React.FC<SearchModalProps> = ({ isOpen = false, children }) => {
    ...
    const tabs = [
        ...
        },
        {
            ...
            elements: [
                ...
                { title: 'Medias', icon: <Image className={`${tabClass}`} /> },
    ...
    return (
        <>
            <div className='cursor-pointer' onClick={handleOpen}>{children}</div>
            {isModalOpen && (
                <ModalLayer setIsOpen={setIsModalOpen} position='top'>
                    ...
                    <div className="mt-4">
                        ...
                        <div className='flex gap-2 items-center'>
                            <Input
                                placeholder='Search ...'
                                className='bg-gray-800  text-white outline-none pl-2 border-none'
                            />
                            ...
                            {tabs.map((tab, i) => (
                                <div key={i} className='flex items-start flex-col gap-3 md:w-64'>
                                    ...
                                    {tab.elements.map(item => (
                                        <div className='py-2 px-4 text-[12px] md:text-sm flex items-center gap-3 w-full transition duration-150 border cursor-pointer hover:bg-[#3e3bfc2c] text-blue-500 rounded-lg hover:border-blue-500 border-gray-800'>
                                            {item.icon}
                                            {item.title}
                                        </div>
                                    ))}
                        ...
                        <div>
                            ...
                            <div className='flex gap-2 my-4 items-center'>
                                {recent.map((rec) => (
                                    <div className='py-1.5 px-4 text-[12px] md:text-sm flex items-center gap-3 transition duration-150 border cursor-pointer hover:bg-[#3e3bfc2c] text-blue-500 rounded-lg hover:border-blue-500 border-gray-800'>
                                        {rec}
