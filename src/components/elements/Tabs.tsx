...
const Tabs: React.FC<TabsProps> = ({ children, defaultTab = 0, orientation = 'horizontal', className }) => {
  ...
  return (
    <div className={cn('flex w-full', orientation === 'vertical' ? 'flex-row' : 'flex-col')}>
      <div className={cn('p-3 rounded-xl bg-noble-black-800 shadow-md', `grid`, gridItems, className)}>
      ...
      <div>
        {Children.map(children, (child, index) => (
          <div key={index}>{index === activeTab && (child as React.ReactElement).props.children}</div>
        ))}
      </div>
    </div>
  )
