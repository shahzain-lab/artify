        ...
        >
            {image && (
                <Image
                    ...
                    height={250}
                    className={cn(
                        position === 'horizontal' ? 'w-[40%] h-full' : 'w-full h-[200px]',
                        " mb-2 rounded-lg"
                    )} />
            )}
            <div className={`${position === 'horizontal' ? 'flex flex-col justify-between p-3 ' : ''}`}>
                ...
                <div className={cn(
                    'grid mt-4 gap-2',
                    (!handler || !handler2) ? 'grid-cols-1' : 'grid-cols-2'
                )}>
                    {handler2 && (
                        <Button onClick={handler2.onClick} size={'sm'} variant={'outline'}>{handler2.title}</Button>
                    )}
                    {handler && (
                        <Button onClick={handler.onClick} size={'sm'}>{handler.title}</Button>
                    )}
                </div>
            </div>
        </div>
